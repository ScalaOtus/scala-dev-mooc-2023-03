...
class futures_sequence_test extends AnyFlatSpec {
  ...
  "full sequence" should "process list of success futures" in {
    ...
    assert(await(module1.futures.task_futures_sequence.fullSequence[Int](List(fut1, fut2, fut3))) === (List(1, 2, 3), List()))
  }
  ...
  it should "process list of success and failures" in {
   ...
   assert(await(module1.futures.task_futures_sequence.fullSequence[Int](List(fut1, failed1, failed2))) === (List(1), List(ex1, ex2)))
  }
  ...
  it should "process list of failures" in {
   ...
   assert(await(module1.futures.task_futures_sequence.fullSequence[Int](List(failed1, failed2))) === (List(), List(ex1, ex2)))
  }
}