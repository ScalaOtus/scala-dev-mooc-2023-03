...
object task_collections extends App {
  ...
  def numbersToNumericString(text: String): String = {
    val digitToStr = Map('0' -> "zero", '1' -> "one", '2' -> "two", '3' -> "three", '4' -> "four", '5' -> "five", '6' -> "six", '7' -> "seven", '8' -> "eight", '9' -> "nine")
    text.flatMap(c => if(c.isDigit) digitToStr.getOrElse(c, "") else c.toString)
  }
