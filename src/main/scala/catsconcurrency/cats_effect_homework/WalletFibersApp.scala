...
object WalletFibersApp extends IOApp.Simple {
  ...
  def run: IO[Unit] =
    for {
      ...
      _ <- Spawn[IO].start(Monad[IO].whileM_(IO(true))(IO.sleep(100.millis) *> wallet1.topup(100)))
      wallet2 <- Wallet.fileWallet[IO]("2")
      _ <- Spawn[IO].start(Monad[IO].whileM_(IO(true))(IO.sleep(500.millis) *> wallet2.topup(100)))
      wallet3 <- Wallet.fileWallet[IO]("3")
      _ <- Spawn[IO].start(Monad[IO].whileM_(IO(true))(IO.sleep(2000.millis) *> wallet3.topup(100)))
      _ <- Spawn[IO].start(Monad[IO].whileM_(IO(true))(for {
        _ <- IO.sleep(1.second)
        b1 <- wallet1.balance
        b2 <- wallet2.balance
        b3 <- wallet3.balance
        _ <- IO.println(b1, b2, b3)
      } yield ()
      ))
      _ <- IO.readLine
    } yield ()
